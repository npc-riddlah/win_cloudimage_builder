#!/bin/bash
PATH_IMG=./result/test.raw				 #Selecting output image path
PATH_MNT=/mnt/win2022stru				 #Selecting PATH where we mount all images (ISO, RAW)
...
PATH_ELEMENT=./elements/win2022_ru_base/		 #Path to first element. You can add more than one. Element - the collection of settings that will be applied to image.
...
./scripts/createImage.bash -i $PATH_IMG -m $PATH_MNT -s $VAR_SIZE -I $PATH_ISO -w $PATH_PE -n "$VAR_NAME" -e $PATH_ELEMENT -u $PATH_UNATTENDXML -r $PATH_RUNNER
